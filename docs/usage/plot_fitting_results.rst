...
.. note::
   The renormalize function could also be done with a ``lambda x:`` function, but it is better to define it as a function to avoid any issues with the scope of the variables (see `here <https://docs.python.org/3/faq/programming.html#why-do-lambdas-defined-in-a-loop-with-different-values-all-return-the-same-result>`_).
