...
Here are shown a simple comparison plot only done with matplotlib functions. On the left, the default matplotlib style is used. On the right, only ``import plothist`` has been added to the script:
